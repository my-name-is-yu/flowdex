...
When Flowdex is triggered for a target task, the parent Codex agent should not
solve that target task directly. The parent may scope enough to author or choose
a workflow, preview/run it, dispatch workers, record worker JSON, continue, and
report from Flowdex output.
...
`collect-results --continue`, then lease the next batch until the durable queue
is empty.
...
- Final reports should be backed by validated claims/evidence whenever making
  factual assertions.
...
subagent judgment; command evidence should come from Flowdex-owned host
commands.
